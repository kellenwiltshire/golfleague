								...
								{completedRounds.map((round, roundIdx) => {
									...
									return (
										<tr key={round.id} className={roundIdx % 2 === 0 ? 'bg-white' : 'bg-gray-50'}>
											<td className='whitespace-nowrap px-6 py-4 text-sm text-gray-500'>{round.date}</td>
											<td className='whitespace-nowrap px-6 py-4 text-sm font-medium text-gray-900'>
												{round.course.name}
											</td>
											<td className='whitespace-nowrap px-6 py-4 text-sm text-gray-500'>{game}</td>
											<td className='whitespace-nowrap px-6 py-4 text-sm text-gray-500'>
												{winningGolfer?.user?.first_name} {winningGolfer?.user?.last_name}
											</td>
											<td className='whitespace-nowrap px-6 py-4 text-sm text-gray-500'>{winningGolfer?.score}</td>
										</tr>
									);
								})}
							</tbody>
